...
def ReadUserPayCountCSV():
    user_pay_count_df = pd.read_csv("../data/process/user_pay_count.txt", sep='\t', encoding='UTF-8', parse_dates=True, index_col=0)
...
def ReadShopOpenDateCSV():
    shop_open_date = pd.read_csv("../data/process/shop_open_date.txt", sep='\t', index_col=0,encoding='UTF-8',parse_dates=[1])
    return shop_open_date;

###读取商家信息
def ReadShopInfoCSV():
    shop_infos = pd.read_csv("../data/original/shop_info.txt",
...
def ReadProcessShopInfoCSV():
    shop_infos = pd.read_csv("../data/process/process_shop_info.txt", sep='\t',encoding='UTF-8', index_col=0)
    return shop_infos;

##读取城市名称对照表
def ReadCityNameCSV():
    city_names = pd.read_csv("../data/process/city_name.txt", index_col=0, encoding='UTF-8',parse_dates=True)
    return city_names;

#读取休假表
def ReadCalendarCSV():
    calendar = pd.read_csv('../data/process/calendar.txt', sep='\t', encoding='UTF-8', index_col=0, parse_dates=True)
    calendar.index = calendar.index.format()
    return calendar;

#读取weekCirlce休假表
def ReadWeekCalendarCSV():
    calendar = pd.read_csv('../data/process/week_circle_calendar.txt', sep='\t', encoding='UTF-8', index_col=0, parse_dates=True)
    calendar.index = calendar.index.format()
    return calendar;
...
def readWeatherData(city):
    weather_info = pd.read_csv("../data/process/weather/process_" + city['pinyin'],
                               index_col=0, encoding='UTF-8',parse_dates=True, dtype={'desc':object, 'wind_level':object})
    return weather_info

#读取浏览信息
def ReadUserViewCSV():
    user_view_count_df = pd.read_csv("../data/process/user_view_count.txt", sep='\t', encoding='UTF-8', parse_dates=True, index_col=0)
