...
#查找上一周对应星期的值 从后往前找。如果上一周没有 则继续往前找
def findPrevMatchWeekDayCount(id, day_time, shop_open_dates, user_pay_counts, calenders):

    open_time = shop_open_dates.loc[id]['date'];
    while True:
        prev_time = day_time - datetime.timedelta(days=6)

        if open_time > prev_time:
            return 0;

        df = base.countShopPayTimePeriods(user_pay_counts, id,
                date_range=[day_time, day_time], time_range=[datetime.timedelta(hours=0), datetime.timedelta(hours=23)])

        if df['count'].values != 0:
            #print(df['count'].values)
            return df['count'].values

        day_time = day_time - datetime.timedelta(weeks=1)



#查找上一周对应daytype类型的值 从后往前找。如果上一周没有 则继续往前找
...
#ARIMA
###=======================================================================================================================================================
def initTrainSingleArimaData(id, shop_open_dates, user_pay_counts, calenders, start_time, end_time):
    open_time = shop_open_dates.loc[id]['date'];
    if open_time > start_time:
        start_time = open_time
    ...
    for time in df.index:
        if df.loc[time]['count'] == 0:
            df.ix[time]['count'] = findPrevMatchWeekDayCount(id, time, shop_open_dates, user_pay_counts, calenders)

    return df['count']

def initTrainArimasData(shop_open_dates, user_pay_counts, calenders, start_time, end_time):
    for id in range(1, 2001):
        print(id)
        arima_df = initTrainSingleArimaData(id, shop_open_dates, user_pay_counts, calenders, start_time, end_time)
        arima_df.to_csv("../data/train/arima/" + str(id), index=False, encoding='UTF-8')


def initPredictArimasData(shop_open_dates, user_pay_counts, calenders, start_time, end_time):
    for id in range(1, 2001):
        print(id)
        arima_df = initTrainSingleArimaData(id, shop_open_dates, user_pay_counts, calenders, start_time, end_time)
        arima_df.to_csv("../data/predict/arima/" + str(id), index=False, encoding='UTF-8')
...
def calculateHolidayMeansData(shop_open_dates, user_pay_counts, calenders, time_ranges, limit_time):
    shop_mean_dict = {}
    for id in range(1,2001):
        print(id)
        mean_dict = {}
        for i, tr in enumerate(time_ranges):
            means = calculateSingleHolidayMeanData(id, shop_open_dates, user_pay_counts, calenders, tr[0], tr[1], limit_time)
            mean_dict[i] = means
...
def calculateSingleHolidayStdData(id, shop_open_dates, user_pay_counts, calenders, start_time, end_time, limit_time):
    #open_time = shop_open_dates.loc[id]['date'];
...
def calculateHolidayStdsData(shop_open_dates, user_pay_counts, calenders, time_ranges, limit_time):
    std_dict = {}
    for id in range(1,2001):
        print(id)

        stds = calculateSingleHolidayStdData(id, shop_open_dates, user_pay_counts, calenders, time_ranges[0], time_ranges[1], limit_time)
        std_dict[id] = stds;
